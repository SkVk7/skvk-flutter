...
import 'package:skvk_application/ui/components/content/content_language_dropdown.dart';
...
class BookData {
  ...
  factory BookData.fromJson(Map<String, dynamic> json) {
    ...
    if (json.containsKey('pages') && json['pages'] != null) {
      ...
      throw Exception(
          'Invalid book format: missing "pages" or "chapters" field',);
    }

    // Process pages data into BookPage objects
    final pages = pagesData.asMap().entries.map((entry) {
...
class BookReaderWidget extends ConsumerStatefulWidget {
  // Callback when content language changes
  ...
  final Function(String)? onContentLanguageChanged;
  ...
  Future<void> _loadBook({bool showLoading = true}) async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (response.statusCode != 200) {
          ...
          try {
            if (bodyBytes.isNotEmpty) {
              ...
              try {
                final errorBody =
                    await ContentDecompressionHelper.extractContent(
                  response,
                  source: 'BookReaderWidget',
                );
          ...
          throw Exception(
              'Failed to load book: $errorMessage (${response.statusCode})',);
        }
        ...
        jsonString = await ContentDecompressionHelper.extractContent(
          response,
          source: 'BookReaderWidget',
        );
      } else {
      ...
      if (!trimmed.startsWith('{') && !trimmed.startsWith('[')) {
        ...
        throw Exception(
            'Invalid book format. Expected JSON but got invalid data.',);
      }
      ...
      try {
        ...
        throw Exception(
            'Invalid book format. Expected JSON but got: ${e.toString()}',);
      }
                              ...
                              children: [
                                ...
                                Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    height:
                                        1.8, // Optimal line spacing for reading
                                    color: ThemeHelpers.getPrimaryTextColor(
                                      context,
                                    ),
                                    fontFamily:
                                ...
                                Padding(
                                  padding: ResponsiveSystem.only(
                                    ...
                                    left: ResponsiveSystem.spacing(
                                      context,
                                      baseSpacing: 60,
                                    ), // Add left padding to avoid overlap with back button
                                  ),
                                  child: Material(
                                    color: Colors.transparent,
                                    child: InkWell(
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: _currentPageIndex > 0
                                              ...
                                              : Colors.transparent,
                                          borderRadius:
                                              ResponsiveSystem.circular(context,
                                                  baseRadius: 24,),
                                        ),
                                        child: Icon(
                                          Icons.chevron_left,

...
class ValidationUtils {
  ...
  static ValidationResult validatePhoneNumber(String phone) {
    ...
    if (!phoneRegex
        .hasMatch(phone.trim().replaceAll(' ', '').replaceAll('-', ''))) {
      return ValidationResult(
        isValid: false,
  ...
  /// Validate age range
  static ValidationResult validateAgeRange(
      DateTime dateOfBirth, int minAge, int maxAge) {
    final now = DateTime.now();
...
mixin FormValidationMixin<T> {
  ...
  bool validateRequiredField(T field, String? value, String fieldName) {
    return validateField(
        field, () => ValidationUtils.validateRequired(value, fieldName));
  }

  /// Validate name field
  bool validateNameField(T field, String name, String fieldName) {
    return validateField(
        field, () => ValidationUtils.validateName(name, fieldName));
  }
  ...
  bool validatePhoneField(T field, String phone) {
    return validateField(
        field, () => ValidationUtils.validatePhoneNumber(phone));
  }

  /// Validate date of birth field
  bool validateDateOfBirthField(T field, DateTime dateOfBirth) {
    return validateField(
        field, () => ValidationUtils.validateDateOfBirth(dateOfBirth));
  }

  /// Validate coordinates field
  bool validateCoordinatesField(T field, double latitude, double longitude) {
    return validateField(
        field, () => ValidationUtils.validateCoordinates(latitude, longitude));
  }
}

...
class AstrologyApiService {
  ...
  Future<Map<String, dynamic>> getBirthData({
    ...
    try {
      ...
      final timeOfBirth = utcBirthDateTime.toIso8601String().split('T')[1].split('.')[0];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey('code') && data.containsKey('message')) {
          ...
          final errorMessage = data['userMessage'] ?? data['message'] ?? 'Unknown API error';
        ...
        try {
          ...
          final errorMessage = errorData['userMessage'] ?? errorData['message'] ?? response.body;
  ...
  Future<Map<String, dynamic>> getCalendarYear({
    ...
    try {
      ...
      final cacheKey = 'year_${year}_${region}_${latitude}_${longitude}_${timezoneId}_$ayanamsha';
  ...
  Future<Map<String, dynamic>> getCalendarMonth({
    ...
    try {
      // Create cache key (includes ayanamsha for accurate nakshatra calculations)
      final cacheKey = 'month_${year}_${month}_${region}_${latitude}_${longitude}_${timezoneId}_$ayanamsha';
  ...
  Duration _getPredictionCacheDuration(String predictionType) {
    switch (predictionType.toLowerCase()) {
      case 'daily':
      case 'day':
        return const Duration(hours: 24); // Same prediction for entire day - 1 API call per day
      case 'dasha':
      case 'dashas':
        return const Duration(hours: 24); // Cache for 1 day to reduce corner case scenarios
      case 'hourly':
      case 'hour':
        return const Duration(hours: 1); // Same prediction for that hour
      case 'transit':
      case 'transits':
        return const Duration(hours: 24); // Transit predictions - cache for 24 hours
      default:
        return const Duration(hours: 24); // Default: 24 hours (1 API call per day)
    }
  }
}

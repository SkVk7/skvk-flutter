...
class DailyPredictionNotificationService {
  ...
  Future<void> showDailyPredictionNotification({
    ...
    try {
      ...
      final generalOutlook = predictions['generalOutlook'] as String? ??
          'Good day ahead';
      ...
      if (moonData != null) {
        ...
        rashi = rashiMap?['name'] as String? ?? rashiMap?['englishName'] as String?;
        nakshatra = nakshatraMap?['name'] as String? ?? nakshatraMap?['englishName'] as String?;
      }
  ...
  Future<bool> _getIsDarkMode() async {
    try {
      ...
      final themeIndex = prefs.getInt('app_theme_mode') ?? AppThemeMode.system.index;
  ...
  String _createNotificationBody({
    ...
    final maxLength = (DesignTokens.fontSizes.bodyMedium * 10).round(); // ~140 chars for medium screens
  ...
  Future<void> showCreateProfileNotification() async {
    try {
      ...
      const body = 'Create your user profile to receive personalized daily astrological predictions and insights.';

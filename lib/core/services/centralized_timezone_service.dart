...
class CentralizedTimezoneService {
  ...
  static CentralizedTimezoneService get instance {
    _instance ??= CentralizedTimezoneService._();
    return _instance!;
  }
  ...
  DateTime convertLocalToUTC(DateTime localTime, double longitude, [double? latitude]) {
  ...
  DateTime _convertLocalToUTCBasic(DateTime localTime, double longitude) {
    ...
    AppLogger.debug('Basic Timezone Conversion:', 'CentralizedTimezoneService');
    AppLogger.debug('Local Time: ${localTime.toIso8601String()}', 'CentralizedTimezoneService');
    AppLogger.debug('Longitude: $longitude°E', 'CentralizedTimezoneService');
    AppLogger.debug(
        'Offset: ${offsetHours.toStringAsFixed(2)} hours', 'CentralizedTimezoneService');
    AppLogger.debug('UTC Time: ${utcTime.toIso8601String()}', 'CentralizedTimezoneService');

    return utcTime;
  }

  /// Enhanced timezone conversion with regional corrections
  DateTime _convertLocalToUTCEnhanced(DateTime localTime, double longitude, double latitude) {
    // Calculate base timezone offset from longitude
    final baseOffsetHours = longitude / 15.0;

    // Apply regional timezone corrections
    double regionalCorrection = 0.0;
    ...
    else if (longitude >= 73.0 && longitude <= 135.0 && latitude >= 18.0 && latitude <= 54.0) {
      regionalCorrection = 8.0; // CST is UTC+8
    }
    // Japan Standard Time (JST) - UTC+9
    else if (longitude >= 129.0 && longitude <= 146.0 && latitude >= 30.0 && latitude <= 46.0) {
      regionalCorrection = 9.0; // JST is UTC+9
    }
    // US Eastern Time (EST/EDT) - UTC-5/-4
    else if (longitude >= -85.0 && longitude <= -66.0 && latitude >= 24.0 && latitude <= 49.0) {
      regionalCorrection = -5.0; // EST is UTC-5 (DST handling not implemented yet)
    }
    // US Pacific Time (PST/PDT) - UTC-8/-7
    else if (longitude >= -125.0 && longitude <= -114.0 && latitude >= 32.0 && latitude <= 49.0) {
      regionalCorrection = -8.0; // PST is UTC-8 (DST handling not implemented yet)
    }
    // UK Time (GMT/BST) - UTC+0/+1
    else if (longitude >= -8.0 && longitude <= 2.0 && latitude >= 50.0 && latitude <= 61.0) {
      regionalCorrection = 0.0; // GMT is UTC+0 (DST handling not implemented yet)
    }

    // Use regional correction if available, otherwise use calculated offset
    final finalOffsetHours = regionalCorrection != 0.0 ? regionalCorrection : baseOffsetHours;
    ...
    // Log the conversion for debugging
    AppLogger.debug('Enhanced Timezone Conversion:', 'CentralizedTimezoneService');
    AppLogger.debug('Local Time: ${localTime.toIso8601String()}', 'CentralizedTimezoneService');
    AppLogger.debug('Location: $latitude°N, $longitude°E', 'CentralizedTimezoneService');
    AppLogger.debug(
        'Regional Correction: ${regionalCorrection != 0.0 ? regionalCorrection.toString() : "None"}',
        'CentralizedTimezoneService');
    AppLogger.debug(
        'Final Offset: ${finalOffsetHours.toStringAsFixed(2)} hours', 'CentralizedTimezoneService');
    AppLogger.debug('UTC Time: ${utcTime.toIso8601String()}', 'CentralizedTimezoneService');
  ...
  DateTime convertForAstrology(DateTime localTime, double longitude, double latitude) {

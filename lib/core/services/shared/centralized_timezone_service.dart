...
class CentralizedTimezoneService {
  ...
  DateTime _convertLocalToUTCBasic(DateTime localTime, double longitude) {
    ...
    _logger.debug('Basic Timezone Conversion:',
        source: 'CentralizedTimezoneService');
    _logger.debug('Local Time: ${localTime.toIso8601String()}',
        source: 'CentralizedTimezoneService');
    _logger.debug('Longitude: $longitude°E',
        source: 'CentralizedTimezoneService');
    _logger.debug('Offset: ${offsetHours.toStringAsFixed(2)} hours',
        source: 'CentralizedTimezoneService');
    _logger.debug('UTC Time: ${utcTime.toIso8601String()}',
        source: 'CentralizedTimezoneService');
  ...
  DateTime _convertLocalToUTCEnhanced(
      DateTime localTime, double longitude, double latitude) {
    ...
    // Log the conversion for debugging
    _logger.debug('Enhanced Timezone Conversion:',
        source: 'CentralizedTimezoneService');
    _logger.debug('Local Time: ${localTime.toIso8601String()}',
        source: 'CentralizedTimezoneService');
    _logger.debug('Location: $latitude°N, $longitude°E',
        source: 'CentralizedTimezoneService');
    _logger.debug(
        'Regional Correction: ${regionalCorrection != 0.0 ? regionalCorrection.toString() : "None"}',
        source: 'CentralizedTimezoneService');
    _logger.debug('Final Offset: ${finalOffsetHours.toStringAsFixed(2)} hours',
        source: 'CentralizedTimezoneService');
    _logger.debug('UTC Time: ${utcTime.toIso8601String()}',
        source: 'CentralizedTimezoneService');

...
class TimezoneUtil {
  ...
  static tz.Location _getLocation(String timezoneId) {
    if (!_initialized) {
      throw StateError('TimezoneUtil not initialized. Call initialize() first.');
    }
    try {
  ...
  static String getTimezoneFromLocation(double latitude, double longitude) {
    ...
    if (latitude >= 32.0 && latitude <= 49.0 && longitude >= -125.0 && longitude <= -114.0) {
      return 'America/Los_Angeles';
    }
    if (latitude >= 50.0 && latitude <= 61.0 && longitude >= -8.0 && longitude <= 2.0) {
